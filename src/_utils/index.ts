...
export const sortByAlphabet = (restaurants: IRestaurantObject[]) => restaurants.sort((a: IRestaurantObject, b: IRestaurantObject) => (
  a.name > b.name ? 1 : -1
));

export const getGenres = (restaurants: IRestaurantObject[]): string[] => restaurants
...
export const getAttire = (restaurants: IRestaurantObject[]): string[] => restaurants
...
export const checkEmptyFilters = (filters: IFilters): boolean => {
  const {
    searchFilter, stateFilter, genreFilter, attireFilter,
  } = filters;
  return (
    stateFilter !== ''
    || genreFilter !== ''
...
export const checkMatches = (place: string, filter: string): boolean => place.toLowerCase().includes(filter.toLowerCase());
...
export const applyFilters = (rests: IRestaurantObject[], filters: IFilters): IRestaurantObject[] => {
  const {
    searchFilter, stateFilter, genreFilter, attireFilter,
  } = filters;
  return rests.filter((rest: IRestaurantObject) => (
